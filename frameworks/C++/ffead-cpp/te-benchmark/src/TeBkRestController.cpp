...
void TeBkRestController::updateCache() {
	...
	DataSourceInterface* sqli = DataSourceManager::getImpl();
	std::string tbName = "cachedworld";
	sqli->startSession(&tbName);
	for (int c = 1; c <= 10000; ++c) {
		GenericObject id;
		id << c;
		TeBkCachedWorld w = sqli->get<TeBkCachedWorld>(id);
...
std::vector<TeBkCachedWorld> TeBkRestController::cachedWorlds(std::string count) {
	int queryCount = 1;
	try {
	...
	std::vector<TeBkCachedWorld> wlst = cchi->mgetO<TeBkCachedWorld>(keys);
